...
***** bins = average of dependnt variable (y) of group of municipalities in the same range of "margin of victory" 
	...
	* local average by bins
	...
	by vote_bins, sort : egen float mean_relief = mean(relief_mv_bp) if vote_bins~=. & alg_mv_bp_party~=.
	...
	* RDD in low drought
	reg relief_mv_bp alg_mv_bp_party mv_bp_party_P1 a_mv_bp_party_P1,r // 
	predict yhat
	predict error, stdp	
	...
	****************
	...
	twoway (scatter mean_relief vote_bins if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party, xline(0) color(blue) msymbol(circle_hollow)) /*
		...
		*/ 	saving("$tmp\_graph_rdd_bp_drought_ignored.gph", replace) 
