...
***** bins = average of dependnt variable (y) of group of municipalities in the same range of "margin of victory" 
	...
	* local average by bins
	...
	by vote_bins, sort : egen float mean_relief_low_bin = mean(relief_mv_bp) if vote_bins~=. & alg_mv_bp_party~=. & low ==1
	by vote_bins, sort : egen float mean_relief_moderate_bin = mean(relief_mv_bp) if vote_bins~=. & alg_mv_bp_party~=.  & moderate ==1
	by vote_bins, sort : egen float mean_relief_severe_bin = mean(relief_mv_bp) if vote_bins~=. & alg_mv_bp_party~=.  & severe ==1
	...
	* RDD in low drought
	reg relief_mv_bp low_aligned mv_bp_party_P1 a_mv_bp_party_P1 if low==1,r // not significant
	predict yhat
	predict error, stdp	
	...
	* RDD in moderate drought
	reg relief_mv_bp moderate_aligned mv_bp_party_P1 a_mv_bp_party_P1 if moderate==1,r // significant
	predict yhat
	predict error, stdp	
	...
	* RDD in severe drought
	reg relief_mv_bp severe_aligned mv_bp_party_P1 a_mv_bp_party_P1 if severe==1,r // not significant
	predict yhat
	predict error, stdp
	...
	****************
	...
	twoway (scatter mean_relief_low_bin vote_bins if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party  & low==1, xline(0) color(blue) msymbol(circle_hollow)) /*
		...
		*/(scatter mean_relief_low_bin vote_bins if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party  & low==1, xline(0) color(red) msymbol(circle_hollow)) /*
		*/(line fitted_values1 mv_bp_party_P1 if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party   & low==1, lpattern(solid) lcolor(black))	/*				
		*/(line lb1 mv_bp_party_P1 if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party   & low==1, lpattern(dash_dot) lcolor(black))	/*
	...
	* save graph 
	...
	graph export "$outdir\graphs\_graph_rdd_bp_low.png", replace	
		
	twoway (scatter mean_relief_moderate_bin vote_bins if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party  & moderate==1, xline(0) color(blue) msymbol(circle_hollow)) /*
		*/(line fitted_values2 mv_bp_party_P1 if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party & moderate==1, lpattern(solid) lcolor(black))	/*
		*/(line lb2 mv_bp_party_P1 if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party  & moderate==1, lpattern(dash_dot) lcolor(black))	/*	
		*/(scatter mean_relief_moderate_bin vote_bins if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party  & moderate==1, xline(0) color(red) msymbol(circle_hollow)) /*
		*/(line fitted_values2 mv_bp_party_P1 if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party   & moderate==1, lpattern(solid) lcolor(black))	/*			
		*/(line lb2 mv_bp_party_P1 if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party   & moderate==1, lpattern(dash_dot) lcolor(black))	/*
		*/(line ub2 mv_bp_party_P1 if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party  & moderate==1, lpattern(dash_dot) lcolor(black))	/*
		*/(line ub2 mv_bp_party_P1 if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party   & moderate==1, lpattern(dash_dot) lcolor(black))	/*	
		*/ , /*
		*/ 	title("") 	/*
	...
	* save graph 
	...
	graph export "$outdir\graphs\_graph_rdd_bp_moderate.png", replace	
		
		
	twoway (scatter mean_relief_severe_bin vote_bins if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party & severe==1, xline(0) color(blue) msymbol(circle_hollow)) /*
		*/(line fitted_values3 mv_bp_party_P1 if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party & severe==1, lpattern(solid) lcolor(black))	/*
		*/(line lb3 mv_bp_party_P1 if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party  & severe==1, lpattern(dash_dot) lcolor(black))	/*
		*/(scatter mean_relief_severe_bin vote_bins if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party  & severe==1, xline(0) color(red) msymbol(circle_hollow)) /*
		*/(line fitted_values3 mv_bp_party_P1 if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party   & severe==1, lpattern(solid) lcolor(black))	/*				
		*/(line lb3 mv_bp_party_P1 if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party   & severe==1, lpattern(dash_dot) lcolor(black))	/*
		*/(line ub3 mv_bp_party_P1 if mv_bp_party_P1 <= 0.0   &   mv_bp_party_P1 >= -$CCT_party  & severe==1, lpattern(dash_dot) lcolor(black))	/*
		*/(line ub3 mv_bp_party_P1 if mv_bp_party_P1 > 0.0   &   mv_bp_party_P1 <= $CCT_party   & severe==1, lpattern(dash_dot) lcolor(black))	/*	
		*/ , /*
		*/ 	title("") 	/*
	...
	* save graph 
	...
	graph export "$outdir\graphs\_graph_rdd_bp_severe.png", replace		
	...
	* save graph 
	graph save Graph "$tmp\_graph_rdd_bp.gph", replace
	graph use "$tmp\_graph_rdd_bp.gph"
	graph export "$outdir\graphs\_graph_rdd_bp.png", replace	
	graph export "$outdir\graphs\_graph_rdd_bp.tif", as(tif) replace
