...
tab drought_quantile
histogram drought_quantile
tab drought_quantile alg_mv_bm_party


* (1) For each quantile-alignment combination, find probability of getting relief: ('ratio' variable below)
gen one=1
bys drought_quantile alg_mv_bm_party: egen denom=sum(one)
drop one
gen one=(relief_mv_bm==1)
bys drought_quantile alg_mv_bm_party: egen num=sum(one)
drop one
gen ratio=num/denom
drop num denom
...
gen temp=ratio if alg_mv_bm_party==1
bys drought_quantile: egen ratio_aligned=max(temp)
drop temp
gen temp=ratio if alg_mv_bm_party==0
...
twoway (scatter diff_ratio drought_quantile if drought_quantile <-0.2 , mcolor(blue) msymbol(diamond_hollow)) || (scatter diff_ratio drought_quantile if drought_quantile >=-0.2 & drought_quantile <=1 , mcolor(black) msymbol(circle_hollow) ) || (scatter diff_ratio drought_quantile if drought_quantile >1 , mcolor(red) msymbol(triangle_hollow) ) /*		*/ , yline(0) xline(-0.1, lpattern(shortdash)) 	/*
	...
	*/ 	saving("$tmp\_graph_ratio_bm_spi.gph", replace)
restore
