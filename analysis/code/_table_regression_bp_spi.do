...
* (1) add controls: mv_bp_party_P1 a_mv_bp_party_P1 --> ALERT: THIS IS OUR FIRST REGRESSION!!!!
reg relief_mv_bp ///
...
outreg2	using "$outdir/regressions/_table_regression_mv_bp_spi.tex", /*
	*/	title("") /*	
	*/	level(95) /*
	*/	dec(3) /*
	*/	fmt(fc) /*
...
/*
In terms of visualization, one suggestion is to show the full estimated coefficients
for the very simple case in which ALL estimated coefficients are present. But then
explain that what we really care is the question:
'Conditional on which type of drought do we find that 'alignment' matters most?',
and for that reason thereafter we only present the t-test as we keep adding controls
and fixed effects. In this way we avoid having to show omitted categories, and the 
message of the paper would be sharp.
*/
