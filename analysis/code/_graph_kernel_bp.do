...
set scheme  s1mono  
twoway  (lpolyci relief_mv_bp drought if alg_mv_bm_party == 1 & drought>=-2.0 & drought<=2.0,  yaxis(1) clpattern(shortdash)  acolor(blue%30)  clcolor(blue) fcolor(blue%30) fintensity(inten20) ) || (lpolyci relief_mv_bp drought if alg_mv_bm_party == 0 & drought>=-2.0 & drought<=2.0, clpattern(longdash)  acolor(red%30)  clcolor(red) fcolor(red%30) fintensity(inten20))  /*
	...
	*/ 	saving("$tmp\_graph_kernel_bp.gph", replace)
