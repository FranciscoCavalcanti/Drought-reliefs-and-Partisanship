...
* variable SPI24_mv_bm represents the SPI (Standardized Precipitation Index (SPI)) during the two years preceding a mayoral election
...
gen drought_spi= -SPI24_mv_bm // positive values of SPI represent droughts
su drought_spi,d

gen drought_spei= -SPEI24_mv_bm // positive values of SPEI represent droughts
su drought_spei,d
...
*** Generate vars:
gen low_spi_aligned=(low_spi==1 & alg_mv_bm_party==1)
gen low_spi_notaligned=(low_spi==1 & alg_mv_bm_party==0)

gen moderate_spi_aligned=(moderate_spi==1 & alg_mv_bm_party==1)
gen moderate_spi_notaligned=(moderate_spi==1 & alg_mv_bm_party==0)

gen severe_spi_aligned=(severe_spi==1 & alg_mv_bm_party==1)
gen severe_spi_notaligned=(severe_spi==1 & alg_mv_bm_party==0)

gen low_spei_aligned=(low_spei==1 & alg_mv_bm_party==1)
gen low_spei_notaligned=(low_spei==1 & alg_mv_bm_party==0)

gen moderate_spei_aligned=(moderate_spei==1 & alg_mv_bm_party==1)
gen moderate_spei_notaligned=(moderate_spei==1 & alg_mv_bm_party==0)

gen severe_spei_aligned=(severe_spei==1 & alg_mv_bm_party==1)
gen severe_spei_notaligned=(severe_spei==1 & alg_mv_bm_party==0)
